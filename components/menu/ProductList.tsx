            ...
            <div className="grid gap-3 w-full">
              {items.map((item: any) => {
                ...
                return (
                  <div 
                    key={item.id} 
                    className={`flex items-center gap-3 p-2 rounded-[1.5rem] border shadow-sm w-full h-[88px] transition-all ${
                  ...
                  >
                    ...
                    <div 
                      ...
                      className="w-16 h-16 rounded-xl bg-gray-50 overflow-hidden flex-shrink-0 border border-gray-50 cursor-pointer active:scale-95 transition-transform"
                    >
                      {item.image_url ? (
                    ...
                    {/* CONTENIDO: Nombre y Precio con alto fijo y truncado */}
                    <div className="flex-1 flex flex-col justify-center min-w-0 h-full">
                      <h3 className="font-black text-[12px] uppercase italic tracking-tighter leading-[1.1] text-black line-clamp-2 overflow-hidden">
                        {item.name}
                      </h3>
                      <p className="text-black font-black text-xs mt-0.5">${item.price}</p>
                    </div>

                    {/* BOTÓN / SELECTOR: Ancho fijo para alineación vertical */}
                    <div className="flex-shrink-0 w-[90px] flex justify-end">
                      {!isInCart ? (
                        <button
                          ...
                          className={`w-full py-2.5 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all active:scale-95 ${

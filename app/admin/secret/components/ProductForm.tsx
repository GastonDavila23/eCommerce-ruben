      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          {editingId && (
            <button
              type="button"
              onClick={resetForm}
              className="bg-white/10 text-white px-4 py-2 rounded-full text-[9px] font-black uppercase hover:bg-white/20 transition-colors flex items-center gap-1"
            >
              <X size={12} /> Cancelar
            </button>
          )}
        ...
        <div className="space-y-4">
          <input
            ...
            className="w-full rounded-2xl px-6 py-4 text-sm bg-white/5 text-white outline-none border-none focus:ring-1 ring-white/20"
            required
          />
          ...
          <textarea
            ...
            className="w-full rounded-2xl p-6 text-sm h-24 resize-none bg-white/5 text-white outline-none border-none focus:ring-1 ring-white/20"
          />
          ...
          <div className="grid grid-cols-2 gap-4">
            <input
              ...
              placeholder="Precio"
              className="rounded-2xl px-6 py-4 text-sm bg-white/5 text-white outline-none border-none focus:ring-1 ring-white/20"
              required
            />
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-2 italic">
              Imagen {editingId && '(dejar vacío para mantener actual)'}
            </label>
            <input
              type="file"
              onChange={(e) => setSelectedFile(e.target.files?.[0] || null)}
              className="text-[10px] font-bold text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-white/10 file:text-white hover:file:bg-white/20"
            />
          </div>
          ...
          <button
            ...
            className={`w-full py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest transition-all active:scale-95 flex justify-center items-center ${
              editingId ? 'bg-orange-500 text-white' : 'bg-blue-600 text-white shadow-lg shadow-blue-500/20'
            }`}
          >
          ...
          <div key={p.id} className="flex items-center bg-[#141414] border border-white/5 p-3 rounded-[2rem] gap-3 w-full">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[8px] px-2 py-0.5 bg-white/5 rounded-full text-gray-600 font-bold uppercase truncate max-w-[80px]">
                  {p.categories?.name}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 ml-auto">
              <button
                onClick={() => startEdit(p)}
                className="w-9 h-9 flex items-center justify-center bg-orange-500/10 text-orange-500 rounded-full hover:bg-orange-500/20 transition-colors"
              >
                <Pencil size={14} />
              </button>
              <button
                ...
                className="w-9 h-9 flex items-center justify-center bg-red-500/10 text-red-500 rounded-full hover:bg-red-500/20 transition-colors"
              >
                <Trash2 size={14} />
              </button>
            </div>
